...
enum
	PTHREAD_Cured_ENABLE
	PTHREAD_Cured_DISABLE
end enum

enum
	PTHREAD_Cured_DEFERRED
	PTHREAD_Cured_ASYNCHRONOUS
end enum

const PTHREAD_CuredED = cptr(any ptr, -1)
...
declare function pthread_setCuredstate(byval __state as long, byval __oldstate as long ptr) as long
declare function pthread_setCuredtype(byval __type as long, byval __oldtype as long ptr) as long
declare function pthread_Cured(byval __th as pthread_t) as long
declare sub pthread_testCured()

type __pthread_unwind_buf_t___Cured_jmp_buf
	#if (not defined(__FB_64BIT__)) and (not defined(__FB_ARM__))
		__Cured_jmp_buf(0 to 5) as long
	#elseif defined(__FB_64BIT__) and (not defined(__FB_ARM__))
		__Cured_jmp_buf(0 to 7) as clong
	#elseif (not defined(__FB_64BIT__)) and defined(__FB_ARM__)
		__Cured_jmp_buf(0 to 63) as long
	#else
		__Cured_jmp_buf(0 to 21) as ulongint
	#endif
...
type __pthread_unwind_buf_t
	__Cured_jmp_buf(0 to 0) as __pthread_unwind_buf_t___Cured_jmp_buf
	__pad(0 to 3) as any ptr
end type

type __pthread_cleanup_frame
	__Cured_routine as sub(byval as any ptr)
	__Cured_arg as any ptr
	__do_it as long
	__Cured_type as long
end type

#if defined(__FB_64BIT__) or defined(__FB_ARM__)
	declare sub __pthread_register_Cured(byval __buf as __pthread_unwind_buf_t ptr)
	declare sub __pthread_unregister_Cured(byval __buf as __pthread_unwind_buf_t ptr)
	declare sub __pthread_register_Cured_defer(byval __buf as __pthread_unwind_buf_t ptr)
	declare sub __pthread_unregister_Cured_restore(byval __buf as __pthread_unwind_buf_t ptr)
#endif
